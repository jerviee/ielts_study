...
words.forEach(w => {
    switch(w.word.toLowerCase()) {
        case 'canadian':
            w.example = 'The Canadian government recently announced comprehensive new policies to support renewable energy development across the entire country.\n加拿大政府最近宣布了全面的新政策，以支持全国范围内的可再生能源发展。\nMany Canadian cities are known for their clean air, friendly people, and stunning natural landscapes.\n许多加拿大城市以其清新的空气、友好的人民和壮丽的自然风光而闻名。';
            break;
    }
});
...
console.log('Fixed examples for canadian');
